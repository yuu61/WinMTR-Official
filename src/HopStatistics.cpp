...
int HopStatistics::GetMax() const
{
	...
	if (max == MAX_HOPS) {
		while ((max > 1) && host_[max - 1].addr.Equals(host_[max - 2].addr) && !host_[max - 1].addr.IsUnspecified()) {
			max--;
		}
	}
	return max;
...
{
	const std::lock_guard g(mutex_);
	return host_[at].returned == 0 ? 0 : static_cast<int>(host_[at].total / static_cast<unsigned long>(host_[at].returned));
}
...
int HopStatistics::GetPercent(int at) const
{
	...
	return (host_[at].xmit == 0) ? 0
	                             : (kPercentScale - (kPercentScale * host_[at].returned / host_[at].xmit));
}
...
void HopStatistics::UpdateRTT(int at, int rtt)
{
	...
	host_[at].last = rtt;
	if (rtt >= 0) {
		host_[at].total += static_cast<unsigned long>(rtt);
	}
	if (host_[at].best > rtt || host_[at].xmit == 1) {
		host_[at].best = rtt;
	}
	// Parenthesise to block the <windows.h> `max` macro from swallowing the call.
	host_[at].worst = (std::max)(host_[at].worst, rtt);
}
