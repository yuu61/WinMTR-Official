...
struct IpAddress {
	...
	{
		if (family == AF_INET) {
			return bytes.v4.s_addr == 0;
		}
		if (family == AF_INET6) {
			return std::ranges::all_of(bytes.v6.u.Byte, [](UCHAR o) { return o == 0; });
		}
		return true;
	}
	...
	[[nodiscard]] bool Equals(const IpAddress& o) const
	{
		...
		if (family == AF_INET) {
			return bytes.v4.s_addr == o.bytes.v4.s_addr;
		}
		if (family == AF_INET6) {
			return memcmp(&bytes.v6, &o.bytes.v6, sizeof(in6_addr)) == 0;
		}
		return true;
	}
