...
namespace {
constexpr UCHAR kIpFlagDontFragment = 0x02;
constexpr DWORD kWaitSlackMs        = 1000;
} // namespace
      ...
      v6_handle_(Icmp6CreateFile())
{
	...
	echo_event_ = CreateEventW(nullptr, FALSE, FALSE, nullptr);
	if (echo_event_ == nullptr) {
...
IcmpIO::~IcmpIO()
{
	...
	// after it is closed.
	if (v4_handle_ != INVALID_HANDLE_VALUE) {
		IcmpCloseHandle(v4_handle_);
	}
	if (v6_handle_ != INVALID_HANDLE_VALUE) {
		IcmpCloseHandle(v6_handle_);
	}
	if (echo_event_ != nullptr) {
		CloseHandle(echo_event_);
	}
}
                     ...
                     HANDLE stop_event, DWORD timeout)
{
	...
	ip_opts.Ttl   = ttl;
	ip_opts.Flags = kIpFlagDontFragment;
	...
	if (dest.family == AF_INET) {
		...
		io_result = IcmpSendEcho2(
		    v4_handle_, echo_event_, nullptr, nullptr,
		...
		io_result = Icmp6SendEcho2(
		    v6_handle_, echo_event_, nullptr, nullptr,
		    &src, &dst,
		    req_data, req_size, &ip_opts,
		    reply_buffer, reply_size, timeout);
	} else {
	...
	const DWORD  n_waits  = (stop_event != nullptr) ? 2 : 1;
	...
	const DWORD wait_ms = timeout + kWaitSlackMs;
